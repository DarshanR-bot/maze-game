...
void start(){
    ...
    loadMedia("GAME OF THRONES",80,"lazy.ttf",{255,255,255}, &gTextTexture);
...
void won(){
    sleep(1);
    background = SDL_LoadBMP("won.bmp");
    SDL_BlitSurface( background, NULL, screenSurface, NULL );
    SDL_UpdateWindowSurface(window);
    win = Mix_LoadWAV("won.wav");
...
void lost(){
    sleep(1);
    background = SDL_LoadBMP("lose.bmp");
    SDL_BlitSurface( background, NULL, screenSurface, NULL );
    SDL_UpdateWindowSurface(window);
    ...
    Tile* tile;
		...
		{	
			...
			{    SDL_Delay(20);
                ...
                //handle punches
                if(punches>0){
                    if(abs(tempx-player.mBox.x)+(tempy-player.mBox.y)<100){
                        player.health-=.15;
                    }
                    punches--;
                    if(player.health<=0){
                ...
                // statusTextTexture.loadFromRenderedText("Location: "+player.statusText[player.status], {255,255,255} ,renderer, font);
