...
class Player
{
    public:
		...
		Player(SDL_Renderer* gRenderer)
        {
            ...
            meal = Mix_LoadWAV("food.wav");
            error = Mix_LoadWAV("error.wav");
            hit = Mix_LoadWAV("punch.wav");
            gLoad = Mix_LoadWAV("loadgun.wav");
            gShot = Mix_LoadWAV("fire.wav");
            vehicle = Mix_LoadWAV("car.wav");
            gun = false;
            dead = false;
            fired = false;
            shot = false;
            ...
            if( e.type == SDL_KEYDOWN )//&& e.key.repeat == 0 )
            {
                ...
                //Adjust the velocity
                    switch( e.key.keysym.sym )
                    {
                        ...
                        case SDLK_SPACE: punch=true; energy-=0.2; Mix_PlayChannel(-1,hit,0);break;
                        case SDLK_RETURN:
                            mVelX +=0;
...
// new@@@@@@@@@@@@@@@@@@@@
            ...
            image = "arm";
            for(int i = 0; i < 3; i++){
                l_a[i] = SDL_CreateTextureFromSurface(gRenderer,SDL_LoadBMP((image+to_string(i)+"l.bmp").c_str()));
                r_a[i] = SDL_CreateTextureFromSurface(gRenderer,SDL_LoadBMP((image+to_string(i)+"r.bmp").c_str()));
                u_a[i] = SDL_CreateTextureFromSurface(gRenderer,SDL_LoadBMP((image+to_string(i)+"u.bmp").c_str()));
                d_a[i] = SDL_CreateTextureFromSurface(gRenderer,SDL_LoadBMP((image+to_string(i)+"d.bmp").c_str()));
            }
            ...
            healthBar= IMG_LoadTexture(gRenderer, "healthBar_new.png");
            energyBar= IMG_LoadTexture(gRenderer, "energyBar.png");
            coinSack = IMG_LoadTexture(gRenderer, "coinSack.png");
        }
